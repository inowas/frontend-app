...
const GameObjectsDetails = (props: IProps) => {
  ...
  const calculateCell = (objective: IObjectiveByObservation) => {
    ...
    const lng =
      ...
      relX * (props.scenario.referencePoints[1][1] - props.scenario.referencePoints[0][1]);
    const lat =
      props.scenario.referencePoints[1][0] -
      relY * (props.scenario.referencePoints[1][0] - props.scenario.referencePoints[0][0]);
    ...
    if (model.rotation % 360 !== 0) {
      g = turf.transformRotate(geometry.toGeoJSON(), -1 * model.rotation, { pivot: model.geometry.centerOfMass });
    }
