...
const SensorDatasourceEditor = (props: IProps) => {
  ...
  const getParametersFromMetadata = (dataSource: SensorDataSource, sensorServerMetaData: ISensorMetaData[]) => {
    ...
    if (
      sensorServerMetaData.filter((s) => s.project === dataSource.project).filter((s) => s.name === dataSource.sensor)
        .length === 0
    ) {
      return [];
    }

    const sensorMetaData = sensorServerMetaData
    ...
    <Modal centered={false} open={true} dimmer={'blurring'}>
      ...
      <Modal.Content>
        <Grid padded={true}>
          ...
          <Grid.Row>
            <Grid.Column>
              <Segment raised={true} loading={fetchingServerMetaData} color={fetchingError ? 'red' : undefined}>
                <Label as={'div'} color={'blue'} ribbon={true}>
                  Metadata
                </Label>
                <Form>
