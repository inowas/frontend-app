...
import RasterFromAzure from './rasterFromAzure';
      ...
      <Segment color="blue">
        ...
        <List>
          ...
          <List.Item>
            ...
            <List.Content>
              X: {metadata.origin[0]}, Y: {metadata.origin[1]}
            </List.Content>
          </List.Item>
          <List.Item>
        ...
        (rData: { hash: string }) => {
          ...
          fetchRasterMetaData(
            ...
            (response: IRasterFileMetadata) => {
              setErrorGridSize(
                interpolation === 11 && (response.rasterXSize !== gridSize.nX || response.rasterYSize !== gridSize.nY)
              );
              setIsLoading(false);
    ...
    <Modal size={'large'} open={true} onClose={props.onCancel} dimmer={'blurring'}>
      ...
      <Modal.Content>
        <Menu pointing={true} secondary={true}>
          <Menu.Item name="GeoTiff" active={activeItem === 'file'} onClick={handleItemClick} value="file" />
          <Menu.Item
          ...
          <Menu.Item
            ...
            value="project"
          />
          <Menu.Item name="Import from CSV" active={activeItem === 'csv'} onClick={handleItemClick} value="csv" />
          {props.parameter.id === 'top' && (
            <Menu.Item
              name="Import Elevation from Azure"
        ...
        {activeItem === 'azure' && <RasterFromAzure onChange={handleChangeRasterFromProject} />}
        {activeItem === 'project' && <RasterFromProject onChange={handleChangeRasterFromProject} />}
        {activeItem === 'csv' && <RasterFromCSV onChange={handleChangeRasterFromProject} unit={props.parameter.unit} />}
        {activeItem === 'interpolation' && (
          <RasterFromPoints onChange={handleChangeRasterFromProject} unit={props.parameter.unit} />
        )}
        {activeItem === 'file' && (
          <Grid divided={'vertically'}>
                ...
                {!isLoading && (
                  <Segment color={'green'}>
                    ...
                    <List bulleted={true}>
                      ...
                      <List.Item>
                        The grid size will be interpolated automatically, if an interpolation method is selected.
                      </List.Item>
                    </List>
              ...
              <Grid.Column>
                ...
                {errorGridSize && (
                  <Message negative={true}>
                    ...
                    <p>
                      The grid size of the uploaded raster does not fit the models grid size. Use an interpolation
                      method or adjust the grid size of the input raster.
                    </p>
                  </Message>
                )}
              </Grid.Column>
            ...
            <Grid.Row columns={2}>
              <Grid.Column>{data && renderBands()}</Grid.Column>
              <Grid.Column>
                {!errorGridSize && data && (
                  <Segment color={'green'}>
                    <RasterDataImage
      ...
      <Modal.Actions>
        ...
        {!errorGridSize && (
          <Button positive={true} onClick={handleClickApply}>
            Apply
          </Button>
        )}
      </Modal.Actions>
