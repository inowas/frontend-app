...
export function discreteRainbowFactory(values) {
    const result = {};
    const colors = ['#393b89', '#5254a3', '#6b6ecf', '#9c9ede', '#637939', '#8ca252', '#b5cf6b', '#cedb9c', '#8c6d31',
        '#bd9e39', '#e7ba52', '#e7cb94', '#843c39', '#ad494a', '#d6616b', '#e7969c', '#7b4173', '#a55194', '#ce6dbd',
        '#de9ed6', '#666666'];

    values.sort((a, b) => a - b).forEach((v, key) => {
        if (colors[key]) {
            result[v] = colors[key];
        } else {
            result[v] = `#${Math.floor(Math.random() * 0x1000000).toString(16).padStart(6)}`;
        }
    });
