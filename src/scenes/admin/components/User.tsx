...
import {updateToolInstanceMetadata} from '../../dashboard/commands';
...
const User = (props: IProps) => {
    ...
    const onIsPrivateCheckboxClick = (tool: string, id: string, name: string, description: string, makePrivate: boolean) => {
        const sc = async () => {
            ...
            try {
                if (makePrivate) {
                    await sendCommandAsync(updateToolInstanceMetadata(tool, {id, name, description, isPublic: false}));
                    setUserDetails({
                ...
                if (!makePrivate) {
                    await sendCommandAsync(updateToolInstanceMetadata(tool, {id, name, description, isPublic: true}));
                    setUserDetails({
                        ...userDetails, tools: userDetails.tools.map((t) => {
                            if (t.id === id) {
                                t.public = true;
                                            ...
                                            <Table.Row key={t.id}>
                                                ...
                                                <Table.Cell>
                                                    <Checkbox
                                                        toggle={true}
                                                        checked={t.public}
                                                        onClick={() => onIsPrivateCheckboxClick(t.tool, t.id, t.name, t.description, t.public)}
                                                    />
                                                </Table.Cell>
                                                <Table.Cell/>
                                            </Table.Row>
