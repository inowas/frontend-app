...
export const documentation = {
    ...
    budgetoutputoption: (
        <div>MODPATH 7 budget output option. Valid budget output options are ‘no’ - individual cell
            ...
            individual cell water balance errors for each time step is printed in the listing file without record
            headers,
            or ‘record_summary’ - a summary of individual cell water balance errors for each time step is printed in the
            listing file with record headers (default is ‘summary’). <b>Only default is implemented.</b>
        </div>
    ),
    endpointfilename: (
    ...
    referencetime: (
        <div>Specified reference time if a float or a list/tuple with a single float value is provided
            (reference time option 1). Otherwise a list or tuple with a zero-based stress period (int) and time step
            (int)
            and a float defining the relative time position in the time step is provided (reference time option 2). If
            referencetime is None, reference time is set to 0 (default is None).<b>Only default is implemented.</b>
        </div>
    ),
    retardation: (
    ...
    retardationfactoroption: (
        <div>If retardationfactoroption is ‘off’, retardation array data are not read and a
            retardation factor of 1 is applied to all cells. If retardationfactoroption is ‘on’, retardation factor
            array
    ...
    stoptime: (
        <div>User-specified value of tracking time at which to stop a particle tracking simulation. Stop time is
            only used if the stop time option is ‘specified’. If stoptime is None amd the stop time option is
            ‘specified’
    ...
    stoptimeoption: (
        <div>String indicating how a particle tracking simulation is terminated based on time. If stop time
            ...
            initial or final steady-state time steps will be extended and all particles will be tracked until they reach
            a
            termination location. If stop time option is ‘specified’, particles will be tracked until they reach a
            termination location or the specified stop time is reached (default is ‘extend’). <b>Only total and
                specified
                are implemented.</b></div>
    ),
    stopzone: (
    ...
    timepointdata: (
        <div>List or tuple with 2 items that is only used if simulationtype is ‘timeseries’ or ‘combined’. If
            the second item is a float then the timepoint data corresponds to time point option 1 and the first entry is
            the
            number of time points (timepointcount) and the second entry is the time point interval. If the second item
            is a
            ...
            user-defined time points. If Timepointdata is None, time point option 1 is specified and the total
            simulation
    ...
    traceparticledata: (
        <div>List or tuple with two ints that define the particle group and particle id (zero-based) of
            the specified particle that is followed in detail. If traceparticledata is None, trace mode is off (default
            is
            None).</div>
    ),
    trackingdirection: (
