...
class BoundaryComparatorMap extends Component<IProps> {
    ...
    public renderBoundaryGeometry(b: Boundary, underlay = false, state = 'noUpdate') {
                ...
                case 'linestring':
                    return (
                        <Polyline
                            ...
                            positions={
                                Geometry
                                    .fromObject(geometry as GeoJson.LineString).coordinatesLatLng as LatLngExpression[]
                            }
                            {...getStyle(state)}
                        />
                    );
    ...
    public renderOtherBoundaries(boundaries: BoundaryCollection) {
        ...
        if (this.props.currentBoundary instanceof Boundary) {
            id = this.props.currentBoundary.id;
        }

        if (this.props.newBoundary instanceof Boundary) {
            id = this.props.newBoundary.id;
        }

        if (id !== null) {
            return boundaries.boundaries
                .filter((b: Boundary) => b.id !== id)
                .map((b: Boundary) => this.renderBoundaryGeometry(b, true));
        }
    }

    public render() {
        const {geometry, currentBoundary, newBoundary, boundaries} = this.props;

        let state = 'noUpdate';

        if (currentBoundary instanceof Boundary
            && newBoundary instanceof Boundary && !currentBoundary.sameAs(newBoundary)) {
            state = 'update';
        }

        if (currentBoundary instanceof Boundary && newBoundary === null) {
            state = 'delete';
        }

        if (currentBoundary === null && newBoundary instanceof Boundary) {
            state = 'add';
        }
        ...
        return (
            ...
            >
                ...
                {currentBoundary && this.renderBoundaryGeometry(currentBoundary)}
                {newBoundary && this.renderBoundaryGeometry(newBoundary, false, state)}
            </Map>
        );
    }
...
export default BoundaryComparatorMap;
