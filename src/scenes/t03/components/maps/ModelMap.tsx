...
const modelMap = (props: IProps) => {
  ...
  const renderOtherBoundaries = (boundaries: BoundaryCollection) => {
    return boundaries.boundaries.map((b: Boundary) => renderBoundaryGeometry(b, true));
  };

  const renderBoundingBox = (boundingBox: BoundingBox) => {
    const data =
      props.rotation && props.geometry
        ? boundingBox.geoJsonWithRotation(props.rotation, props.geometry.centerOfMass)
        : boundingBox.geoJson;
    return <GeoJSON key={md5(JSON.stringify(data))} data={data} style={getStyle('bounding_box')} />;
  };

  return (
    <MapContainer style={style.map} zoomControl={false} bounds={props.geometry.getBoundsLatLng()}>
      <BasicTileLayer />
