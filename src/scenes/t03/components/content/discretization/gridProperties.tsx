...
const GridProperties = (props: IProps) => {
  ...
  useEffect(() => {
    setCellSize([
      Math.round(dyCell(BoundingBox.fromGeoJson(boundingBox as AllGeoJSON), GridSize.fromObject(gridSize)) * 10000) /
        10,
      Math.round(dxCell(BoundingBox.fromGeoJson(boundingBox as AllGeoJSON), GridSize.fromObject(gridSize)) * 10000) /
        10,
    ]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gridSize]);
  ...
  const calculateRotation = (r: number) => {
    ...
    // With rotation:
    const withRotation = turf.transformRotate(props.geometry.toGeoJSON(), -1 * r, {
      pivot: props.geometry.centerOfMass,
    });
  ...
  const handleChangeRotation = (value: number, remainder?: [number, number]) => {
    const withRotation = turf.transformRotate(props.geometry.toGeoJSON(), -1 * value, {
      pivot: props.geometry.centerOfMass,
    });
  ...
  const renderCells = () => {
    ...
    return (
      <Grid>
        <Grid.Row>
          <Grid.Column width={8}>
            <Button onClick={handleClickRedo} icon={true} fluid={true} labelPosition="left">
              <Icon name="redo" />
              Redo
            </Button>
          </Grid.Column>
          <Grid.Column width={8}>
            <Button primary={true} onClick={handleClickApply} icon={true} fluid={true} labelPosition="left">
              <Icon name="save" />
              Apply
            </Button>
          </Grid.Column>
        ...
        <Grid.Row>
          <MapContainer style={style.map} bounds={props.boundingBox.getBoundsLatLng()}>
            <BasicTileLayer />
            <GeoJSON key={uniqueId()} data={boundingBoxRotated} style={getStyle('bounding_box')} />
            <AffectedCellsLayer
    ...
    return (
      <Form>
        <Grid>
          ...
          <Grid.Row>
            <Grid.Column width={3}>
              <Form.Input label="Rotation angle" value={rotation} />
            </Grid.Column>
          ...
          <Grid.Row>
            <Grid.Column width={3}>
              <Form.Input label="Intersection" value={intersection} />
            </Grid.Column>
            <Grid.Column width={13} verticalAlign="middle">
              <label>&nbsp;</label>
              <SliderWithTooltip
  ...
  return (
    <React.Fragment>
      <Form>
        <Form.Group>
          <Form.Input label="Rows" value={gridSize.n_y} readOnly={true} style={{ width: '150px' }} />
          <Form.Input label="Columns" value={gridSize.n_x} readOnly={true} style={{ width: '150px' }} />
          <Form.Input label="Grid rotation" value={props.rotation || 0} readOnly={true} style={{ width: '150px' }} />
          <Form.Input label="Intersection" value={props.intersection || 0} readOnly={true} style={{ width: '150px' }} />
          <Form.Button
            fluid={true}
            icon="pencil"
            label="&nbsp;"
      ...
      <Modal open={showModal} onClose={handleToggleModal}>
        ...
        <Modal.Content>{cells ? renderCells() : renderPreview()}</Modal.Content>
      </Modal>
    </React.Fragment>
  );
};
