...
const CalculationProcess = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const f = async (model: ModflowModel) => {
      ...
      dispatch(
        props.updateCalculation(Calculation.fromCalculationIdAndState(hash, CALCULATION_STATE_UPDATING_PACKAGES))
      );
      ...
      try {
        ...
        dispatch(props.updateCalculation(Calculation.fromCalculationIdAndState(hash, CALCULATION_STATE_SENDING_DATA)));
        await sendModflowCalculationRequest(p);
        dispatch(
          props.updateCalculation(
            Calculation.fromCalculationIdAndState(hash, CALCULATION_STATE_WAITING_FOR_CALCULATION)
          )
        );
      } catch (e) {
        dispatch(
