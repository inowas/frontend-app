...
export const getActiveCellFromCoordinate = (
  ...
  if (x < boundingBox.xMin || x > boundingBox.xMax || y < boundingBox.yMin || y > boundingBox.yMax) {
    return null;
  }
...
export const calculateActiveCells = (
  ...
  if (geometry.fromType('point')) {
    ...
    const cell = getActiveCellFromCoordinate(coordinate as number[], boundingBox, gridSize);
    if (cell) {
      activeCells.addCell(cell);
    }
  }
...
export const getRowsAndColumnsFromCoordinate = (coordinate: any, boundingBox: BoundingBox, gridSize: GridSize) => {
  ...
  return {
    columns: cell ? [gridSize.distX[cell[0]]] : [],
    rows: cell ? [gridSize.distY[gridSize.distY.length - cell[1] - 1]] : [],
  };
};
